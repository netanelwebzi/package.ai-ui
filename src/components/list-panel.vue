				...
				<md-list-item v-for="(item, index) in filteredItems" @click="selectItem(item, index)"
					...
					<div class="item-container">
						...
						<div class="container-action-icons">
							...
							<span v-if="(onPhaseExport() || onPhaseJenny()) && (item.state != 'POSTPONED' && item.errors.length == 0)" style="font-size:10px;">{{ item.startTime.substr(0, 5) + '-' + item.finishTime.substr(0, 5) }}</span>
						</div>
					</div>
